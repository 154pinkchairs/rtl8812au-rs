...
fn start_bss_network(padapter: &mut _adapter, pbuf: u8){
    &p: u8;
    let val8: u8;
    let cur_channel: u8;
    let cur_bwmode: u8;
    let cur_ch_offset: u8;
    let bcn_interval: u16;
    let acparm: u32;
    let ie_len: i32;
    let pregpriv: &mut registry_priv = &mut padapter.registrypriv;
    let pmlmepriv: &mut mlme_priv = &mut padapter.mlmepriv;
    let psecuritypriv: &mut security_priv = &mut padapter.securitypriv;
    let pnetwork: &WLAN_BSSID_EX = &mut pmlmepriv.cur_network.network;
    let pmlmeext: &mut mlme_ext_priv = &mut padapter.mlmeextpriv;
    let pmlmeinfo: &mut mlme_ext_info = &mut pmlmeext.mlmext_info;
    let pnetwork_mlmeext: &WLAN_BSSID_EX = &mut pmlmeinfo.network;
    let pht_info: &HT_info_element = &mut HT_info_element::default();
    #[cfg(CONFIG_P2P)]{
        let pwdinfo: &mut wifidirect_info = &mut padapter.wdinfo;
    }
    let cbw40_enable: u8 = 0;
    let change_band: u8 = _FALSE;

    let bcn_interval: u16 = pnetwork.Configuration.BeaconPeriod;
    let cur_channel = pnetwork.Configuration.DSConfig;
    let cur_bwmode = CHANNEL_WIDTH_20;
    let cur_ch_offset = HAL_PRIME_CHNL_OFFSET_DONT_CARE;

    // check if there is wps ie,
    // if there is wpsie in beacon, the hostapd will update beacon twice when stating hostapd,
    // and at first time the security ie ( RSN/WPA IE) will not include in beacon.

    if rtw_get_wps_ie(pnetwork.IEs.as_ptr().offset(_FIXED_IE_LENGTH_ as isize), pnetwork.IELength - _FIXED_IE_LENGTH_, None, None).is_null() {
        pmlmeext.bstart_bss = true;
    }
    // TODO: update wmm, ht cap
    // pmlmeinfo.WMM_enable;
    // pmlmeinfo.ht_enable;
    if plmempriv.qospriv.qos_option{
...
if pmlmepriv.cur_network.join_res == false //setting only at first time
{
    ...
    //set EDCA param reg
    #[cfg(feature = "CONFIG_CONCURRENT_MODE")]{
        acparm = 0x005ea42b;
    }
    #[cfg(not(feature = "CONFIG_CONCURRENT_MODE"))]{
        acparm = 0x002F3217; // VO
    }
    rtw_hal_set_hwreg(padapter, HW_VAR_AC_PARAM_VO, &acparm);
    acparm = 0x005E4317; // VI
    rtw_hal_set_hwreg(padapter, HW_VAR_AC_PARAM_VI, &acparm);
    acparm = 0x005ea42b;
    rtw_hal_set_hwreg(padapter, HW_VAR_AC_PARAM_BE, &acparm);
    acparm = 0x0000A444; // BK
    rtw_hal_set_hwreg(padapter, HW_VAR_AC_PARAM_BK, &acparm);
    ...
    rtw_hal_set_hwreg(padapter, HW_VAR_BEACON_INTERVAL, &bcn_interval);

    if pmlmepriv.cur_network.join_res == false {
        //reset TSF
        #[cfg(feature = "CONFIG_CONCURRENT_MODE")]{
            if padapter.adapter_type > PRIMARY_ADAPTER {
                if rtw_buddy_adapter_up(padapter) {
                    let pbuddy_adapter: &mut _adapter = padapter.pbuddy_adapter;
                    // turn on all dynamic functions on PRIMARY_ADAPTER, dynamic functions only
                    // run at PRIMARY_ADAPTER
        ...
        #[cfg(feature = "CONFIG_80211N_HT")]{
            if rtw_get_ie(pnetwork.IEs.as_ptr().offset(_FIXED_IE_LENGTH_ as isize), _HT_ADD_INFO_IE_ as u8, &ie_len, pnetwork.IELength - _FIXED_IE_LENGTH_).is_null() == false {
                pht_info = (pnetwork.IEs.as_ptr().offset(_FIXED_IE_LENGTH_ as isize) + ie_len + 2) as *mut HT_info_element;
                cbw40_enable = pht_info.cbw40_en;
            }
            if cbw40_enable && pht_info.infos[0] {
                if pht_info.infos[0] == 1 {
                    cur_ch_offset = HAL_PRIME_CHNL_OFFSET_LOWER;
                } else if pht_info.infos[0] == 3 {
                    cur_ch_offset = HAL_PRIME_CHNL_OFFSET_UPPER;
                }
                cur_bwmode = CHANNEL_WIDTH_40;
                switch (pht_info.infos[0] as u32) {
                    1 => {
                        cur_ch_offset = HAL_PRIME_CHNL_OFFSET_LOWER;
                    }
                    3 => {
